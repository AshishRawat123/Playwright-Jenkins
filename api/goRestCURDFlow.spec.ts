...
test(`get user by id `, async ({ request }) => {
    ...
    expect(responseWithTime.respone.status()).toBe(200);
...
test('Update user details that we pushed', async({request})=>{
    ...
    const obj = {
        headers : AuthHeader,
        body: {
            "name": fullName, "status": "inactive" 
        }
    }

    const responseWithTime = await apiRequest(request,'put',`/public/v2/users/${firstUserID}`,obj);
    expect([200,201]).toContain(responseWithTime.respone.status());

    //validate now
    const resp = await defaultGetRequest(request,`/public/v2/users/${firstUserID}`);
